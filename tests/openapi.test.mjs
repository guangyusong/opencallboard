...
test("published OpenAPI contract covers the hosted product boundaries", () => {
  ...
  assert.equal(spec.servers[0].url, "https://app.opencallboard.com");
  ...
  assert.match(spec.paths["/api/communication-outbox/{id}/release-approval"].post.description, /never calls the provider directly/i);
  assert.match(spec.paths["/api/communication-reminders/evaluate"].post.description, /live-delivery flag/i);
  ...
  assert.deepEqual(spec["x-callboard-safety"], {
    externalEmailDelivery: true,
    emailReleaseRequiresOneTimeApproval: true,
    scheduledReminderDelivery: true,
