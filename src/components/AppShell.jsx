  ...
  return (
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      <aside className="sidebar">
        ...
        <button className="event-switcher" onClick={() => onNavigate("/settings")}><span>{data.event.initials}</span><div><b>{data.event.shortName}</b><small>{data.event.dates}</small></div><ChevronDown size={18} /></button>
